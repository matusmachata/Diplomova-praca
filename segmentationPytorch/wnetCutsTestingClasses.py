...
model_path = "/kaggle/input/asdfasdf/pytorch/default/1/wnet_ex.pt"
...
class OIADDRDatasetAllChannels(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "2-class":
            if self.foreground_source != None:
...
def reconstruct_mask_from_patches(dataset, idx, patch_size=64):
    _, mask_patches = dataset[idx]  # Get mask patches for image at index
    mask_patches = [mp.numpy() if isinstance(mp, torch.Tensor) else mp for mp in mask_patches]
        ...
        for x in range(0, W, step):
            if ((x // step) + (y // step)) % 2 == 0:
                if x + patch_size <= W and y + patch_size <= H:
                    patch = mask_patches[patch_idx]
                    mask_canvas[y:y + patch_size, x:x + patch_size] += patch
                    weight_map[y:y + patch_size, x:x + patch_size] += 1.0
                    patch_idx += 1

    # Avoid division by zero
    weight_map[weight_map == 0] = 1
    reconstructed_mask = mask_canvas / weight_map

    # Convert to integer mask if needed (e.g., round to nearest class)
    reconstructed_mask = np.rint(reconstructed_mask).astype(np.uint8)
    return reconstructed_mask


def reconstruct_prediction_from_patches(pred_patches, image_size=(448, 448), patch_size=64):
    pred_patches = pred_patches.detach().cpu()

    if pred_patches.shape[1] == 1:
        # Binary segmentation — squeeze channel dimension
        pred_patches = pred_patches.squeeze(1)
        pred_patches = pred_patches.numpy()
    else:
        # Multi-class — take argmax over channels
        pred_patches = torch.argmax(pred_patches, dim=1).numpy()
    ...
    for y in range(0, H, step):
        for x in range(0, W, step):
            if ((x // step) + (y // step)) % 2 == 0:
                if x + patch_size <= W and y + patch_size <= H:
                    patch = pred_patches[patch_idx]
                    mask_canvas[y:y + patch_size, x:x + patch_size] += patch
                    weight_map[y:y + patch_size, x:x + patch_size] += 1.0
                    patch_idx += 1

    weight_map[weight_map == 0] = 1
    reconstructed = mask_canvas / weight_map

    return np.rint(reconstructed).astype(np.uint8)
...
pred_patches = (pred_patches > 0.9).float()

full_image = reconstruct_image_from_patches(image_patches, patch_size, output_size)
# full_true_mask = reconstruct_mask_from_patches(mask_patches, patch_size, output_size)
# full_pred_mask = reconstruct_mask_from_patches(pred_patches, patch_size, output_size)
full_true_mask = reconstruct_mask_from_patches(test_dataset, index, patch_size=patch_size)
full_pred_mask = reconstruct_prediction_from_patches(pred_patches, image_size=output_size, patch_size=patch_size)
# full_pred_mask = reconstruct_mask_from_patches(test_dataset, index, patch_size=patch_size)

print("Unique classes in predicted mask:", torch.unique(torch.tensor(full_pred_mask)))

axs[0].imshow(full_image, cmap="gray")
axs[0].set_title("Originálny obrázok")
axs[1].imshow(full_true_mask, cmap='jet', interpolation='nearest', vmin=0, vmax=2)
axs[1].set_title("Správna maska")
axs[2].imshow(full_pred_mask, cmap='jet', interpolation='nearest', vmin=0, vmax=2)
axs[2].set_title("Predikovaná maska")
