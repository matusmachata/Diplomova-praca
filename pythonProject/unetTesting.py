...
model_path = "/kaggle/input/sadfsdfs/pytorch/default/1/DeepLabV3Plus.pt"
...
class Wnet(pl.LightningModule):
    def __init__(self, freeze_encoder=freeze_encoder, unfreeze_at_epoch=20):
        ...
        self.model = smp.DeepLabV3Plus(
            ...
            in_channels=1 if not allChannels else 3,
            classes=1
        )
    ...
    def _freeze_encoder(self):
        ...
        for param in self.model.encoder.parameters():  # Access encoder through `model`
    ...
    def on_test_epoch_end(self):
        ...
        print(f"Final Testing loss: {(sum(self.test_loss_history) / len(self.test_loss_history)):.4f}")
        print(f"Final Testing IoU: {(sum(self.test_iou_history) / len(self.test_iou_history)):.4f}")
        print(f"Final Testing F1: {(sum(self.test_f1_history) / len(self.test_f1_history)):.4f}")
        print(f"Final False Positive Rate: {false_positive_rate:.4f}%")

    def count_frozen_layers(self):
        # Access encoder layers through the model's `encoder`
        total_layers = len(list(self.model.encoder.parameters()))  # Use `model.encoder`
        frozen_layers = sum(
            1 for param in self.model.encoder.parameters() if not param.requires_grad)  # Use `model.encoder`
...
# Plot 10 random images with masks
numOfImages = 5
num_channels = 3
indices = random.sample(range(len(test_dataset)), numOfImages)
fig, axs = plt.subplots(numOfImages, 3, figsize=(12, numOfImages * 4))
...
for i, idx in enumerate(indices):
    ...
    if num_channels == 1:
        axs[i, 0].imshow(img.squeeze().numpy(), cmap="gray")
    else:
        axs[i, 0].imshow(img.permute(1, 2, 0).numpy())  # Convert (C, H, W) to (H, W, C)

    axs[i, 0].set_title("Originálny obrázok")
    axs[i, 1].imshow(true_mask.squeeze().numpy(), cmap="gray")
    axs[i, 1].set_title("Správna maska")
    axs[i, 2].imshow(pred_mask.squeeze().cpu().numpy(), cmap="gray")
    axs[i, 2].set_title("Predikovaná maska")
