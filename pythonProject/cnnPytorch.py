...
class SegmentationModel(pl.LightningModule):
    ...
    def _unfreeze_encoder(self, layers_to_unfreeze=1):
        ...
        for idx, child in enumerate(self.model.encoder.children()):
            ...
            if unfrozen_count >= layers_to_unfreeze:
                break  # Stop after unfreezing the desired number of layers
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("testing_dataset_iou", dataset_iou2, prog_bar=True)
        # self.log("testing_per_image_iou", per_image_iou, prog_bar=True)
        # self.log("testing_dataset_iou", dataset_iou, prog_bar=True)
    ...
    def count_frozen_layers(self):
        total_layers = len(list(self.model.encoder.parameters()))
        frozen_layers = sum(1 for param in self.model.encoder.parameters() if not param.requires_grad)
        return total_layers, frozen_layers, total_layers - frozen_layers

    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.parameters(), lr=lr)
        scheduler = lr_scheduler.CosineAnnealingLR(optimizer, T_max=int(T_MAX / 4), eta_min=eta_min)
        return {
            "optimizer": optimizer,
